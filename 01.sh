...
error() {
  ...
  echo "Usage: $0 [-t <mode>] [-p <mode>] [-w] [-m] [-h]"
  echo "At least one of -t, -p, -w, -m or -h must be specified."
  echo "Modes for -t and -p: 1, 2 or 3."
  exit 1 
}
...
# extraire la colonne de la date/heure et de la température
cut -d ";" -f 2,11 $file > date_temp.csv

# trier les données par date/heure
sort -t ";" -k 1 date_temp.csv > date_temp_sorted.csv

# produire la moyenne des températures par date/heure
awk -F ";" 'NR > 1 {
  dates[$1]+=$2
  n[$1]++
} END {
...
rm date_temp.csv date_temp_sorted.csv
	...
	OFMT="%.2f"
	awk -F ";" 'NR > 1 { dates[$2 " " $1] += $11; count[$2 " " $1]++ } 
END { 
...
rm date_press.csv date_press_sorted.csv
	...
	OFMT="%.2f"
	awk -F ";" 'NR > 1 { dates[$2 " " $1] += $7; count[$2 " " $1]++ } 
END { 
  for (date_station in dates) { 
    split(date_station, parts, " ") 
     avg = dates[date_station] / count[date_station]
...
#verifier si les cases vides on un effet.
